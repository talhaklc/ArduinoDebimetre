...
unsigned char flowsensor = 2; // Sensor Giriş Pini
...
void loop ()
{
 Serial.println(emmemiktari);
    sondurum=sayac-yenisayac;
   if(sondurum>30){
    emmemiktari++;
   }
   yenisayac=sayac;
...
void debi(){
   ...
   {
      flowRate = ((1000.0 / (millis() - cloopTime)) * flow_frequency) / 7.5;
      ...
      l_hour = (flow_frequency * 60 / 7.5); // (Darbe frekansı x 60 dk) / 7.5Q = Akış Hızı Litre/saat
